...
static struct usb_endpoint_descriptor usbnet_ss_intr_out_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_OUT,
	.bmAttributes = USB_ENDPOINT_XFER_INT,
...
static struct usb_ss_ep_comp_descriptor usbnet_ss_intr_out_comp_desc = {
	.bLength =		sizeof usbnet_ss_intr_out_comp_desc,
	.bDescriptorType =	USB_DT_SS_ENDPOINT_COMP,
	.wBytesPerInterval =	__constant_cpu_to_le16(16),

};

static struct usb_descriptor_header *usbnet_ss_function[] = {
	(struct usb_descriptor_header *) &usbnet_intf_desc,
	...
	(struct usb_descriptor_header *) &usbnet_ss_intr_out_desc,
	(struct usb_descriptor_header *) &usbnet_ss_intr_out_comp_desc,
	NULL,
};
*/
			...
			struct usb_function *f)
{
	...
	if (gadget_is_superspeed(cdev->gadget)) {
		...
		usbnet_ss_bulk_out_desc.bEndpointAddress =
		    usbnet_fs_bulk_out_desc.bEndpointAddress;
		usbnet_ss_intr_out_desc.bEndpointAddress =
		    usbnet_fs_intr_out_desc.bEndpointAddress;
	}
	*/
...
static void usbnet_do_set_config(u16 new_config, struct usb_function *f)
{
	int result = 0;
	unsigned long flags;
	struct usb_request *req;
//	int high_speed_flag = 0;
	...
	if (new_config == 1) { /* Enable End points */
		...
		result = config_ep_by_speed(cdev->gadget, f, g_usbnet_context->bulk_in);

		if (result != 0) {
			g_usbnet_context->bulk_in->desc = NULL;
			printk(KERN_INFO "%s:  config_ep_by_speed ret=%d\n",
			      __func__, result);
			return;
		}
		...
		result = config_ep_by_speed(cdev->gadget, f, g_usbnet_context->bulk_out);
		if (result != 0) {
			g_usbnet_context->bulk_out->desc = NULL;
			printk(KERN_INFO "%s:  config_ep_by_speed ret=%d\n",
			      __func__, result);
			usb_ep_disable(g_usbnet_context->bulk_in);
			return;
		...
		result = config_ep_by_speed(cdev->gadget, f, g_usbnet_context->intr_out);
		if (result != 0) {
			g_usbnet_context->intr_out->desc = NULL;
			printk(KERN_INFO "%s:  config_ep_by_speed ret=%d\n",
			      __func__, result);
			usb_ep_disable(g_usbnet_context->bulk_in);
			usb_ep_disable(g_usbnet_context->bulk_out);
...
int usbnet_bind_config(struct usb_configuration *c)
{
	...
	//LS2_USB tarial added descriptor for SSUSB support
	//dev->function.ss_descriptors = usbnet_ss_function;
