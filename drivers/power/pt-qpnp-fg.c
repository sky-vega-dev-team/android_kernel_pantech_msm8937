...
// ef71
#define SKY_SOC_EMPTY	2510		// 2.51 X 1000    0.76039 X 1000
#define SKY_SOC_FULL	95416	// 95.41651 X 1000
#define SKY_DEFAULT_RCOMP	0x70 // 112
#define SKY_TEMP_CO_HOT		425 // -0.425
#define SKY_TEMP_CO_COLD	-6275 // -6.275
...
{	
	u16 data;
	data = (msb<<8)|lsb;
	max17058_write_reg(chip_client, MAX17058_RCOMP_MSB, data);
}
EXPORT_SYMBOL(pt_max17058_set_rcomp);

static int pt_max17058_get_vcell(struct fg_chip *chip, int *batt_vol)
{
	int rc=0;
	u8 msb;
	u8 lsb;
	...
	do {
		rc = max17058_read_reg(chip->client, MAX17058_VCELL_MSB, buf, 2);
		if(rc == -EIO)	
			msleep(10);
	}while(rc < 0 && retry-- > 0);
...
static int pt_max17058_get_soc(struct fg_chip *chip, int *batt_soc)
{
	...
	do {
		rc = max17058_read_reg(chip->client, MAX17058_SOC_MSB, buf, 2);
		if(rc == -EIO)
			msleep(10);
	}while(rc < 0 && retry-- > 0);
...
#define MAX17058_DEFAULT_FG_SOC 50
int pt_max17058_get_fg_soc(void)
{
	struct power_supply *bms_psy;
	struct fg_chip *chip;
	...
	if (!bms_psy) {
		...
		return MAX17058_DEFAULT_FG_SOC;
	}
	chip = container_of(bms_psy, struct fg_chip, bms_psy);
	
	return chip->max17058_fg_soc;
}
EXPORT_SYMBOL_GPL(pt_max17058_get_fg_soc);
...
static int get_prop_capacity(struct fg_chip *chip)
{
	...
	pt_max17058_get_soc(chip,&batt_soc);	
	return batt_soc;
}

static int get_prop_voltage(struct fg_chip *chip)
{
	unsigned int batt_vol;

	pt_max17058_get_vcell(chip, &batt_vol);
	return batt_vol;
}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = get_prop_voltage(chip);
		break;
...
#if defined(MAXIM_FG)
static int  pt_max17058_battery_probe(struct i2c_client *client,
...
static int  pt_max17058_battery_remove(struct i2c_client *client)
{
...
static const struct dev_pm_ops pt_max17058_pm_ops = {
};
...
static struct i2c_driver pt_max17058_i2c_driver = {
	.driver	= {
		...
		.pm  		= &pt_max17058_pm_ops,	
	},
	.probe		= pt_max17058_battery_probe,
	.remove		= pt_max17058_battery_remove,
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	if (rc < 0) {
		pr_err("MAX17058 F/G I2C add driver failed %d", rc);
		return rc;
	}
	chip->client = chip_client;
	rc = fg_of_init(chip);
	if (rc) {
	...
	chip->max17058_fg_soc = MAX17058_DEFAULT_FG_SOC;
