...
#define MXT_PATCH_LOCK_CHECK		0
#define MXT_PATCH_SUPP_CHECK		0
...
#if MXT_PATCH_T9STATUS_CHK
#define MXT_XML_SRC_T9STATUS	"T9STATUS"	//10
#endif
#define MXT_XML_SRC_USER1			"USER1"		//used_noise_status
#define MXT_XML_SRC_USER2			"USER2"		//used
#define MXT_XML_SRC_USER3			"USER3"		//used
#define MXT_XML_SRC_USER4			"USER4"		//used
#define MXT_XML_SRC_USER5			"USER5"		//used_touchmode
#define MXT_XML_SRC_USER6			"USER6"		//used_fingerprint_status
#define MXT_XML_SRC_USER7			"USER7"		//17
#define MXT_XML_SRC_USER8			"USER8"		//18
#define MXT_XML_SRC_USER9			"USER9"		//19
#define MXT_XML_SRC_USER10			"USER10"		//20
#define MXT_XML_SRC_USER11			"USER11"		//21
#define MXT_XML_SRC_USER12			"USER12"		//22
#define MXT_XML_SRC_USER13			"USER13"		//23
#define MXT_XML_SRC_USER14			"USER14"		//24
#define MXT_XML_SRC_USER15			"USER15"		//25
#define MXT_XML_SRC_USER16			"USER16"		//26
#define MXT_XML_SRC_USER17			"USER17"		//27
#define MXT_XML_SRC_USER18			"USER18"		//28
#define MXT_XML_SRC_USER19			"USER19"		//29
#define MXT_XML_SRC_USER20			"USER20"		//30
...
enum {
	MXT_PATCH_EVENT_TA_PLUGGED =  0,
	MXT_PATCH_EVENT_TA_UNPLUGGED,
	MXT_PATCH_EVENT_KNOCKON,
	MXT_PATCH_EVENT_END
};
...
{
	mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 0x01u, false);
}
...
static int mxt_patch_start_timer(struct mxt_data *data, u16 period)
{
	...
	ret = mxt_write_mem(data->client, object->start_address+
			...
			struct touch_pos* tpos, u8 tid, u16 x, u16 y, u8 finger_cnt)
{
	...
	if(cal_condition){
	    int error = mxt_read_object(data,
	    			MXT_SPT_DYNAMICCFGCONTAINER_T71,
...
static void mxt_patch_load_t71data(struct mxt_data *data)
{
	...
	obj = mxt_get_object(data,
			MXT_SPT_DYNAMICCFGCONTAINER_T71);
	if(obj){
	    error = mxt_read_mem(data->client, obj->start_address,
...
#endif
		MXT_XML_SRC_USER1,	//MXT_PATCH_ITEM_USER1	11
		MXT_XML_SRC_USER2,	//MXT_PATCH_ITEM_USER2	12
		MXT_XML_SRC_USER3,	//MXT_PATCH_ITEM_USER3	13
		MXT_XML_SRC_USER4,	//MXT_PATCH_ITEM_USER4	14
		MXT_XML_SRC_USER5,	//MXT_PATCH_ITEM_USER5	15
		MXT_XML_SRC_USER6,	//MXT_PATCH_ITEM_USER6	16
		MXT_XML_SRC_USER7,	//MXT_PATCH_ITEM_USER6	17
		MXT_XML_SRC_USER8,	//MXT_PATCH_ITEM_USER6	18
		MXT_XML_SRC_USER9,	//MXT_PATCH_ITEM_USER6	19
		MXT_XML_SRC_USER10,	//MXT_PATCH_ITEM_USER6	20
		MXT_XML_SRC_USER11,	//MXT_PATCH_ITEM_USER6	21
		MXT_XML_SRC_USER12,	//MXT_PATCH_ITEM_USER6	22
		MXT_XML_SRC_USER13,	//MXT_PATCH_ITEM_USER6	23
		MXT_XML_SRC_USER14,	//MXT_PATCH_ITEM_USER6	24
		MXT_XML_SRC_USER15,	//MXT_PATCH_ITEM_USER6	25
		MXT_XML_SRC_USER16,	//MXT_PATCH_ITEM_USER6	26
		MXT_XML_SRC_USER17,	//MXT_PATCH_ITEM_USER6	27
		MXT_XML_SRC_USER18,	//MXT_PATCH_ITEM_USER6	28
		MXT_XML_SRC_USER19,	//MXT_PATCH_ITEM_USER6	29
		MXT_XML_SRC_USER20,	//MXT_PATCH_ITEM_USER6	30
	};
				...
				bool do_action)
{
	if(do_action){
		__mxt_patch_debug(data, "TA:%d FCNT:%d AREA:%d AMP:%d"
			" SUM:%d TCH:%d ATCH:%d KCNT:%d KVAL:%d T9STATUS:%d U1:%d U2:%d U3:%d U4:%d U5:%d U6:%d \n",
		...
		u16* pevent_addr)
{
	...
	dev_err(&data->client->dev, "PATCH MAGIC:%X SIZE:%d DATE:%d"
...
static void mxt_patch_init_tsrc(struct test_src* tsrc)
{
	...
	tsrc->charger=data_common->charging_mode;
	tsrc->finger_cnt=-1;
	tsrc->area=-1;
	tsrc->amp=-1;
	tsrc->sum_size=-1;
	...
	tsrc->user8 = t255_user[7];
	tsrc->user9 = t255_user[8];
...
//			(data->fingers[i].state != MXT_STATE_RELEASE))
		if ((fingerInfo[i].status != TOUCH_EVENT_INACTIVE) &&
...
//						data->fingers[i].x, data->fingers[i].y, finger_cnt);
						fingerInfo[i].x, fingerInfo[i].y, finger_cnt);
				}
			}
		}
		else{
...
static void mxt_patch_T72_object(struct mxt_data *data,
		struct mxt_message *message)
{
	u8 *msg = message->message;
	u8 nState = (msg[1] & 0x07);
	static u8 preState = -1;
	struct device *dev = &data->client->dev;

	if(preState == nState)
		return;

	dev_err(dev, "preState %d nState %d\n", preState, nState);
		...
		struct mxt_message *message)
{
	...
	switch (type) {
		...
		case MXT_PROCG_NOISESUPPRESSION_T72:
			mxt_patch_T72_object(data, message);
			break;
...
static int mxt_patch_init(struct mxt_data *data, u8* ppatch)
{
	...
	if(!ppatch){
		dev_err(&data->client->dev, "%s patch file error\n", __func__);
		return 1;
	}
	...
	if(!patch_size){
		dev_err(&data->client->dev, "%s patch_size error\n", __func__);
		return 1;
	}
	ppheader = (struct patch_header*)ppatch;
	...
	if (!patch_info->src_item) {
		dev_err(&data->client->dev, "src_item alloc error\n");
		return 1;
	}
