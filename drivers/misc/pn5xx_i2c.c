...
//#define FEATURE_PN5XX_KERNEL_LOG
#define FEATURE_PN5XX_IRQ_IN_SUSPEND

#define FEATURE_PN5XX_USE_PMIC_CLK
#endif

#ifdef FEATURE_PN5XX_IRQ_IN_SUSPEND
#include <linux/wakelock.h>
#endif

#ifdef FEATURE_PN5XX_USE_PMIC_CLK
#include <linux/clk.h>
#endif
...
#if defined(FEATURE_PN5XX_IRQ_IN_SUSPEND)	
    struct wake_lock    pn5xx_wake_lock;
#endif	
#ifdef FEATURE_PN5XX_USE_PMIC_CLK
    struct clk         *s_clk;
#endif
...
#ifdef FEATURE_PN5XX_KERNEL_LOG
    pr_info("==> %s #2 (ven, firm, irq)=(%d, %d, %d)\n", __func__, gpio_get_value(pn5xx_dev->ven_gpio), gpio_get_value(pn5xx_dev->firm_gpio), gpio_get_value(pn5xx_dev->irq_gpio));
#endif
                           ...
                           struct pn5xx_i2c_platform_data *pdata)
{
    ...
    val = of_get_named_gpio_flags(node, "nxp,ven-gpio", 0, &flags);
    if (val >= 0) {
        pdata->ven_gpio = val;
    }
    else{
    ...
    val = of_get_named_gpio_flags(node, "nxp,firm-gpio", 0, &flags);
    if (val >= 0) {
        pdata->firm_gpio = val;
    }
    else {
    ...
    val = of_get_named_gpio_flags(node, "nxp,irq-gpio", 0, &flags);
    if (val >= 0) {
        pdata->irq_gpio = val;
    }
    else {
...
#endif
{
    ...
    if (gpio_is_valid(pdata->clkreq_gpio)) {
        ...
        #endif
        ret = gpio_request(pdata->clkreq_gpio, "nxp,pn5xx-clkreq");
        if (ret){
...
*/
...
#if defined(FEATURE_PN5XX_IRQ_IN_SUSPEND)
    wake_lock_init(&pn5xx_dev->pn5xx_wake_lock, WAKE_LOCK_SUSPEND, CHIP);
#endif
    ...
    if (IS_ERR(pn5xx_dev->s_clk))
    {
        dev_err(&client->dev,"[%s] clk_get err \n", __func__);
    }
    rc = clk_prepare_enable(pn5xx_dev->s_clk);
    if (rc)
    {
    	dev_err(&client->dev,"[%s] clk_prepare_enable fail rc[%d]\n", __func__, rc);
    }
#endif /* FEATURE_PN5XX_USE_PMIC_CLK */
...
err_misc_register:
    ...
    kfree(pn5xx_dev);
#if defined(FEATURE_PN5XX_IRQ_IN_SUSPEND)
    wake_lock_destroy(&pn5xx_dev->pn5xx_wake_lock);
#endif	
err_exit:
    if (gpio_is_valid(pdata->clkreq_gpio))
...
static int pn5xx_suspend(struct device *dev)
{   
	struct pn5xx_dev *pn5xx_dev = dev_get_drvdata(dev);
#ifdef FEATURE_PN5XX_KERNEL_LOG
	pr_info("pn5xx_suspend*********** client IRQ[%d]\n", pn5xx_dev->client->irq);
	pr_info("pn5xx_suspend*********** gpio_to_irq IRQ[%d]\n", gpio_to_irq(pn5xx_dev->irq_gpio));
#endif

    if(gpio_get_value(pn5xx_dev->ven_gpio) == 1)
    {
...
static int pn5xx_resume(struct device *dev)
{    
	struct pn5xx_dev *pn5xx_dev = dev_get_drvdata(dev);
	
#ifdef FEATURE_PN5XX_KERNEL_LOG
	pr_info("<************pn5xx_resume client IRQ[%d]\n", pn5xx_dev->client->irq);
	pr_info("<************pn5xx_resume gpio_to_irq IRQ[%d]\n", gpio_to_irq(pn5xx_dev->irq_gpio));
#endif

    if(gpio_get_value(pn5xx_dev->ven_gpio) == 1)
 	{
	 	if(gpio_get_value(pn5xx_dev->irq_gpio) == 1)
	 	{
            wake_lock_timeout(&pn5xx_dev->pn5xx_wake_lock, 2*HZ);
	 	}
        irq_set_irq_wake(pn5xx_dev->client->irq, 0);
 	}
	return 0;
}
static struct dev_pm_ops pn5xx_pm_ops = 
{ 
