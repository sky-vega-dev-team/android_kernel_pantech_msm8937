...
#ifdef CONFIG_PANTECH_SND_QCOM_PATCH //20160601 BS1@SND - Defense code for avoiding kernel panic when Media server was died in VoLTE / VT call
    if (prtd->state == VOIP_STOPPED) {
        pr_err("%s: [SND]VOIP state is VOIP_STOPPED, avoid kernel panic! \n", __func__);
        return -ERESTARTSYS;
    }
#endif       
